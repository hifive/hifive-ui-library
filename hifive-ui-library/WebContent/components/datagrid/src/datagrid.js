...
//				}
			...
			applyData: function (data) {
				...
				var original = util.mapObject(data, function(value, key) {
					var originalValue = util.hasProperty(originalValues, key) ? originalValues[key] : value;
					return {
						key: key,
						value: originalValue,
					};
				});
				var edited = util.mapObject(data, function(value, key) {
					var newValue = util.hasProperty(newValues, key) ? newValues[key] : value;
					return {
						key: key,
						value: newValue,
					};
...
(function () {
	...
	var CellSelector = defineClass(NAMESPACE + '.CellSelector', function (ctx) {
		...
		var cellSelectorDefinition = {
			 ...
			 * @param {Index} row
			 * @param {Index} rowLength
			 * @param {Index} column
			 * @param {Index} columnLength
			 */
			 selectStart: function (row, rowLength, column, columnLength) {
			 ...
			 * @param {Index} columnLength
			 */
			selectMove: function (row, rowLength, column, columnLength) {
				if (!this._isSelecting) {
				...
				var newRange = {
					...
					rowLength: Math.abs(startRow - row) + rowLength,
					columnIndex: Math.min(startColumn, column),
					columnLength: Math.abs(startColumn - column) + columnLength
				};
			 ...
			 * @param {Index} columnLength
			 */
			focus: function (row, rowLength, column, columnLength) {
				this._focusedCell = {
					row: row,
					column: column
				};
	...
	var PropertyGridDataMapper = defineClass(NAMESPACE + '.PropertyGridDataMapper', function (ctx) {
		...
		var propertyDataGridMapperDefinition = {
			...
			/**
			 * TODO このグリッドでは必ず縦方向が行、横方向が列だが、このメソッド名は
			 * このルールに従っていないのでPropertyNumなどと直すのがよい
			 * @private
			 */
			...
			/**
			 * インデックスのオフセットを計算します。ここでいうオフセットとは、
			 * セルが結合されている場合に、可視範囲の先頭の行番号と結合の境界の行番号の差のことです。
			 * @param startRow
			 * @param startColumn
			 * @param depth
			...
			/**
			 *  あるデータ行に対応するセル範囲について、セルの結合状態を計算します。
			 *  結果は2次元配列で表現され、各要素は、結合されていない、または結合の親セルならtrue、
			 *  結合されて隠れているセル（ファントムセル）の場合はfalseです。
			 *
			 * @private
	...
	var gridLogicDefinition = {
		...
		focusCell: function (row, column) {
			...
			// 指定位置が結合セル内の場合は、セルの左上の位置を検索
			var exists = false;
			for (var c = column; c >= 0; c--) {
				for (var r = row; r >= 0; r--) {
					var cell = this.getGridCell(r, c);
					var rowSpan = cell.rowSpan;
					var colSpan = cell.colSpan;
					if (rowSpan && colSpan) {
						var endRow = r + rowSpan - 1;
						var endColumn = c + colSpan - 1;
						if ((r <= row && row <= endRow) &&
							(c <= column && column <= endColumn)) {
							rowLength = rowSpan;
							columnLength = colSpan;
							exists = true;
							break;
						}
					}
			...
			this._cellSelector.focus(rowIndex, rowLength, columnIndex, columnLength);
			},
		...
		selectRangeStart: function (row, column) {
			var rowLength = this.getGridCell(row, column).rowSpan;
			var columnLength = this.getGridCell(row, column).colSpan;
			this._cellSelector.selectStart(row, rowLength, column, columnLength);
		},

		selectRangeMove: function (row, column) {
			var rowLength = this.getGridCell(row, column).rowSpan;
			var columnLength = this.getGridCell(row, column).colSpan;
			this._cellSelector.selectMove(row, rowLength, column, columnLength);
		},
		...
		selectRangeUp: function () {
			...
			var cell = this.getGridCell(rowIndex, columnIndex);
			var rowSpan = cell.rowSpan;
			var colSpan = cell.colSpan;
			
			var focusCell = this.getGridCell(focus.row, focus.column);
			var focusRowSpan = focusCell.rowSpan;
			var focusColSpan = focusCell.colSpan;
			
			// 選択範囲の結合セル数最大値を取得
			var maxRowSpan = 1;
			for(var c = columnIndex; c <= columnIndex + columnLength - 1; c++) {
				cell = this.getGridCell(rowIndex, c);
				rowSpan = cell.rowSpan;
				if(rowSpan && maxRowSpan < rowSpan) {
					maxRowSpan = rowSpan;
				}
			}

			// 一番上の単一結合セル選択時
			if(maxRowSpan !== 1  && maxRowSpan === rowLength) {
				return rowIndex;
			}

			var result;

			if (focus.row + focusRowSpan - 1 === range.rowIndex + maxRowSpan - 1 && 1 < range.rowLength) {
				// フォーカスの下を範囲選択時
				rowLength -= 1;
				...
				// 上方向探索
				for(var c = columnIndex; c <= columnIndex + columnLength - 1; c++) {
					// フォーカス範囲選択時に結合セルだった場合はindex変更
					cell = this.getGridCell(rowIndex, c);
					if(cell.isPhantom) {
						for (var r = rowIndex - 1; r >= 0; r--) {
							cell = this.getGridCell(r, c);
							rowSpan = cell.rowSpan;
				...
				// 左右探索
				for(var r = rowIndex; r <= rowIndex + rowLength - 1; r++) {
					// 先頭カラム
					var firstColIndex = columnIndex;
					cell = this.getGridCell(r, firstColIndex);
					colSpan = cell.colSpan;
					if(cell.isPhantom) {
						for(var c = firstColIndex - 1; c >= 0; c--) {
							cell = this.getGridCell(r, c);
							colSpan = cell.colSpan;
					...
					// 最終カラム
					var lastColIndex = columnIndex + columnLength - 1;
					cell = this.getGridCell(r, lastColIndex);
					colSpan = cell.colSpan;
					if(colSpan && colSpan !== 1) {
						columnLength += colSpan - 1;
					}
		...
		selectRangeDown: function () {
			...
			if (range.rowIndex + maxRowSpan - 1 < focus.row) {
				// フォーカスの上を範囲選択時

				// 取得した最大結合行分Index加算
				rowIndex += maxRowSpan;
				rowLength -= maxRowSpan;
				result = rowIndex;
		...
		selectRangeLeft: function () {
			...
			for(var r = rowIndex; r <= rowIndex + rowLength - 1; r++) {
				// フォーカス範囲選択時に結合セルだった場合はindex変更
				cell = this.getGridCell(r, columnIndex);
				colSpan = cell.colSpan;
				if(colSpan && maxColSpan < colSpan) {
					maxColSpan = colSpan;
				}
			}

			if (focus.column <= range.columnIndex + maxColSpan - 1 && maxColSpan < range.columnLength) {
				// フォーカスの右を範囲選択時
				columnLength -= 1;
				...
				for(var c = columnIndex; c <= columnIndex + columnLength - 1; c++) {
					...
					cell = this.getGridCell(firstRowIndex, c);
					if(cell.isPhantom) {
						for(var r = firstRowIndex - 1; r >= 0; r--) {
							cell = this.getGridCell(r, c);
							rowSpan = cell.rowSpan;
							if(rowSpan && rowSpan !== 1) {
								rowIndex -= rowSpan - 1;
					...
					// 最終行
					var lastRowIndex = rowIndex + rowLength - 1;
					cell = this.getGridCell(lastRowIndex, c);
					rowSpan = cell.rowSpan;
					if(rowSpan && rowSpan !== 1) {
						rowLength += rowSpan - 1;
					}
		...
		selectRangeRight: function () {
			...
			if (range.columnIndex + colSpan - 1 < focus.column) {
				// フォーカスの左を範囲選択時
				...
				columnIndex += maxColSpan;
				columnLength -= maxColSpan;
				result = columnIndex;
...
(function () {
	...
	var tableGridViewController = {
		...
		'.gridCellFrame mousedown': function (context, $el) {
			...
			if (!$target.is($el) && $target.parent().is(':input') && !$target.parent().is(':disabled')) {
				return;
			}
			if ($target.is('a')) {
				return;
		...
		_triggerKeydownArrow: function (event) {
			...
			var totalRows = this._gridLogic.getTotalRows();
			var totalColumns = this._gridLogic.getTotalColumns();
			var rowSpan = this._gridLogic.getGridCell(row, column).rowSpan;
			var colSpan = this._gridLogic.getGridCell(row, column).colSpan;
		...
		_triggerKeydownTab : function(event) {
			...
			column += event.shiftKey ? -1 : colSpan;
...
(function () {
	...
	var gridController = {
		 ...
		 */
		isSelectedDataAll: function () {
			return this._gridLogic.isSelectedDataAll();
		},
