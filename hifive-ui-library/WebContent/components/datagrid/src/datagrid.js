...
(function() {
   ...
   * クラスを継承させます。
   * <p>
   * IE8 との互換性のため constructor プロパティの enumerable 属性を指定できないため、 for-in に constructor プロパティがでてきてしまうことに注意してください。
   * </p>
   * 
   * @private
   */
   ...
   * @param {function(ClassContext) : {constructorFunction: function(new), superConstructorFunction: function(new)=, mixins: Array.<Mixin>=, abstractMethods:
   *          Array.<string>=, definition: Object}} defineFunction クラス定義に必要な情報を返す関数
   ...
   * @memberOf h5.ui.components.datagrid.util
   * @param {string} className クラスの名前
   * @param {function(ClassContext) : {constructorFunction: function(new), superConstructorFunction: function(new)=, mixins: Array.<Mixin>=, definition:
...
(function() {
   ...
   * @param {function(Array.<*>=, Array.<*>): Array.<*>} [options.aggregateArgsFunction] 引数のまとめ方（デフォルトは最新で上書き）
  ...
  var EventListenerSet = util.defineClass(NAMESPACE + '.EventListenerSet', function(ctx) {
    ...
    var eventListenerSetDefinition = {
       ...
       * イベントリスナをまとめて登録します。
       * 
       * @param {Object.<string, Object.<string, EventListener>>} listenersDefinition イベントリスナの定義オブジェクト
       */
      ...
      removeAllListeners : function() {
        ...
        for (var i = 0, len = listeners.length; i < len; i++) {
          ...
          log.debug('REMOVE eventListener: type="{0}"; {1} -> {2}', entry.type, util.className(entry.dispatcher), util.className(this._context));
  ...
  var Throttle = util.defineClass(NAMESPACE + '.Throttle', function(ctx) {
     ...
     * @param {boolean} [option.deferInWaiting=false] 待機中の呼び出しで更に呼び出しを遅延させるか
     * @param {boolean} [option.ignoreInCooling=false] 実行後 wait 時間経つまでの呼び出しを無視するか
     * @param {function(Array.<*>=, Array.<*>): Array.<*>} [option.aggregateArgsFunction] 引数のまとめ方（デフォルトは最新で上書き）
     */
...
(function() {
   ...
   * </p>
   * 
   * @typedef {PartialMatchFilterUIString|DistinctValuesFilterUIString|ValueArrayFilterUIParam} FilterUIParam
   */
...
(function() {
  ...
  var AbortablePromise = util.defineClass(NAMESPACE + '.AbortablePromise', function(ctx) {
     ...
     * @class 中止を指示することが可能な Promise です。失敗時のコールバックでは中断されたことによる失敗かを知るために {@link AbortablePromise.FailResult FailResult} 型のオブジェクトが渡されます。
    ...
    var editDefinition = {
      ...
      addData : function(data) {
        ...
        if (util.hasProperty(this._addedDataSet, dataId)) {
          throw error.DuplicateDataId.createError(dataId);
        }
      ...
      applyData : function(data) {
        ...
        if (util.hasProperty(this._addedDataSet, dataId)) {
          return {
            ...
            editStatus : 'added',
            original : null,
            edited : this._addedDataSet[dataId]
          };
        }
  ...
  var LocalDataAccessor = util.defineClass(NAMESPACE + '.LocalDataAccessor', function(ctx) {
    ...
    var localDataAccessorDefinition = {
      ...
      setSourceDataSet : function(sourceDataSet) {
        var array = [];

        util.forEach(sourceDataSet, this.own(function(data) {
          array.push(data);
        }));
        this.setSourceDataArray(array);
      },

      setSourceDataArray : function(sourceDataArray) {
        this._sourceDataArray = sourceDataArray;
      },
  ...
  var AjaxDataAccessor = util.defineClass(NAMESPACE + '.AjaxDataAccessor', function(ctx) {
    ...
    var ajaxDataAccessorDefinition = {
      ...
      search : function(searchParam) {
        ...
        returnValidator.run('searchSettingMaker の戻り値', 'result', setting, type.validateAjaxSetting);
        ...
        return promise.then(this.own(function(/* var_args */) {
          ...
          returnValidator.run('searchResultParser の戻り値', 'result', result, type.validateSearchResult);
          return result;
        }));
      },
      ...
      fetch : function(fetchParam, fetchRange) {
        ...
        returnValidator.run('fetchSettingMaker の戻り値', 'result', setting, type.validateAjaxSetting);
        ...
        return promise.then(this.own(function(/* var_args */) {
          ...
          returnValidator.run('fetchResultParser の戻り値', 'result', result, type.validateFetchResult);
          return result;
        }));
      },
      ...
      find : function(dataId) {
        ...
        returnValidator.run('findSettingMaker の戻り値', 'result', setting, type.validateAjaxSetting);
        ...
        return promise.then(this.own(function(/* var_args */) {
          ...
          returnValidator.run('findResultParser の戻り値', 'result', result, type.validateData);
          return result;
        }));
      },
      ...
      commit : function(edit) {
        ...
        returnValidator.run('commitSettingMaker の戻り値', 'result', setting, type.validateAjaxSetting);
  ...
  var AllFetchSearcher = util.defineClass(NAMESPACE + '.AllFetchSearcher', function(ctx) {
    ...
    var allFetchSearcherDefinition = {
      ...
      _refreshCache : function(searchParam) {
        ...
        util.forEach(this._originalDataArray, this.own(function(data) {
          var editedData = this._dataSource.applyEdit(data.original);
          editedDataSet[editedData.dataId] = editedData;

          // MEMO: 最初からあるデータを削除するとソート時にエラーが起きる。
          // dataArrayの削除したデータのインデックス要素にnullが入っており、それにアクセスしているため。
          // editedData が null ならば dataArray に追加しないよう変更
          if (editedData.edited == null) {
            return;
          }

          dataArray.push(editedData.edited);
        }));
        ...
        var searchedArray = searchDataArray(dataArray, idProperty, searchParam);

        var wrapedArray = util.map(searchedArray, function(data) {
          var dataId = data[idProperty];
          return editedDataSet[dataId];
        });

        this._dataSet = util.mapObject(editedDataSet, function(data, id) {
          return {
            key : id,
            value : data.edited
          };
      ...
      _updateCache : function(searchedDataArray) {
        ...
        this._idToIndex = {};
        this._cache = searchedDataArray;
        util.forEach(searchedDataArray, this.own(function(searched, index) {
          this._idToIndex[searched.dataId] = index;
        }));
      },
      ...
      _addData : function(changeEvent, fromServer) {
        ...
        var newData = {
          ...
          editStatus : fromServer ? 'unchanged' : 'added',
          original : null,
          edited : edited
        };
...
(function() {
  ...
  var CellSelector = defineClass(NAMESPACE + '.CellSelector', function(ctx) {
     ...
     * @mixes Disposable
     */
    function CellSelector(getGridCell) {
      this._isSelecting = false;
      this._focusedCell = null;
      this._selectedRange = null;
      this._getGridCell = getGridCell;
    }
    ...
    var arrayCellSizeSetDefinition = {
      ...
      toPositionFromIndex : function(index) {
        ...
        if (this._positionArray.length <= index) {
          // 範囲外が指定された場合は、サイズ全体を返す。
          // データ行方向にスクロールした時、一番最後付近にスクロールしようとすると
          // indexが範囲を超える（正確には、lengthと同じになる）場合があり、
          // その場合にエラーにならないようにする対策。
      ...
      toSizeFromIndex : function(index) {
        ...
        if (this._sizeArray.length <= index) {
          // 指定されたインデックスが配列を超えている場合、
          // 配列の一番最後の値を返すこととする。
          // SingleValueの方は、インデックスを超えた場合にも常に固定サイズを返しており、
          // それに相当する動作としてこの動きにする。
          // 本質的に、インデックスがlengthと一致する（＝範囲を超える）のは
          // ピクセルスクロールが原因。
          return this._sizeArray[this._sizeArray.length - 1];
        }
  ...
  var PixelScrollCalculator = defineClass(NAMESPACE + '.PixelScrollCalculator', function(ctx) {
    ...
    var pixelScrollCalculatorDefinition = {
      ...
      calcDisplayRange : function(displayAreaSize, position) {
        ...
        var isLast = false;
        // TODO テーブルのボーダー幅に従うようにする
        var TABLE_BORDER_WIDTH = 1;

        // 旧実装ではスクロールの最終位置に来た場合に displayPosition: 'last'を返し、
        // setHorizontalPositionなどでright:0（またはbottom:0）としていたが、
        // この方法だと中の要素(table)をtop, leftで動かしている場合に
        // スクロール末尾に移動した時に表示位置が不正になるので
        // 常にスクロール量を計算するようにした。
        if (endPosition <= position + TABLE_BORDER_WIDTH) {
          isLast = true;
        }
        ...
        return {
          index : index,
          length : length,
          displayPosition : displayPosition + (isLast ? -TABLE_BORDER_WIDTH : 0)
        };
      },
  ...
  var GridRange = util.defineClass(NAMESPACE + '.GridRange', function(ctx) {
    ...
    function GridRange(param) {
      ...
      this._offsetIndex = param.offsetIndex;
    }
  ...
  var PropertyGridDataMapper = defineClass(NAMESPACE + '.PropertyGridDataMapper', function(ctx) {
    ...
    function PropertyGridDataMapper(searcher, dataFocus, dataSelector, cellSelector, param) {
      ...
      var headerDepth = this._headerProperties.getMaxDepth();
      var dataDirSize = this._param.dataDirectionSize.size;

      var dataDirSizeArray;
      if (this._param.dataDirectionSize.headerSizeArray != null) {
        dataDirSizeArray = this._param.dataDirectionSize.headerSizeArray.slice(0);
      } else {
        dataDirSizeArray = [];
        for (var i = 0; i < headerDepth; i++) {
          dataDirSizeArray.push(dataDirSize);
        }
      }

      var cells = this._getDataDirectionCells();

      for (var i = 0, len = cells - headerDepth; i < len; i++) {
        dataDirSizeArray.push(dataDirSize);
      }
      this._dataDirectionSizeSet = new ArrayCellSizeSet(dataDirSizeArray, dataDirSize);
    ...
    var propertyDataGridMapperDefinition = {
      ...
      request : function(requestParam) {
        ...
        util.forEach(requestParam, this.own(function(range) {
          ...
          if (startIndex == null) {
            ...
            if (endIndex < index + dLength) {
              endIndex = index + dLength; // TODO endIndexにdepthが考慮されていない
            }
          }
        }));
        ...
        /* rowCount追加への対応 */
        if (this._param.rowCount != null && length > 0) {
          var idx;
          var len;
          if (typeof this._param.rowCount === 'function') {
            idx = this._param.rowToDataIndex(startIndex, this._searcher);
            len = this._param.rowToDataIndex(endIndex, this._searcher) - idx;
          } else {
            // rowCountはnullでなく、かつfunctionではない = Numberである
            // この場合、rowToDataIndex()は指定していなくてよい
            var fixedRowCount = this._param.rowCount;
            idx = Math.floor(startIndex / fixedRowCount);
            len = Math.floor(endIndex / fixedRowCount) - idx;
          }

          startIndex = idx;
          length = len;

          var dataCount = this.getSearcher().getCount();
          if (startIndex + length > dataCount) {
            // データの範囲を取得しようとした場合は長さを調整する
            length = dataCount - startIndex;
          }
        }
        ...
        var rangeSet = this._mapRangeSet(requestParam, searchParam, dataReferences, startIndex);
        ...
        var fetchFunction = this.own(function() {
          ...
          return promise.then(this.own(function(fetchResult) {
            ...
            return this._mapRangeSet(requestParam, searchParam, dataReferences, startIndex);
          }));
        });
      ...
      getPropertySize : function(propertyName) {
        var validator = ctx.argsValidator('public');
        validator.arg('propertyName', propertyName, type.validatePropertyName);

        var size = this._propertySizeHolder.getSize(propertyName);
        return size;
      },
      ...
      _changeCount : function() {
        ...
        if (typeof this._dataDirectionSizeSet.setCells === 'function') {
          // SingleValueCellSizeSetの場合はsetCellsがある
          this._dataDirectionSizeSet.setCells(cells);
        } else if (typeof this._dataDirectionSizeSet.setSizeArray === 'function') {
          // ArrayCellSizeSetの場合はsetCellsがないので、配列をセットしなおす
          var headerDepth = this._headerProperties.getMaxDepth();
          var dataRowSize = this._param.dataDirectionSize.size;

          var sizeArray;
          if (this._param.dataDirectionSize.headerSizeArray != null) {
            sizeArray = this._param.dataDirectionSize.headerSizeArray.slice(0);
          } else {
            sizeArray = [];
            for (var i = 0; i < headerDepth; i++) {
              sizeArray.push(dataRowSize);
            }
          }
          ...
          this._dataDirectionSizeSet.setSizeArray(sizeArray);
        }

        this.dispatchEvent({
          type : 'changeDataCount'
        });
      },
      ...
      /**
       * @private
       * @type {Object.<PropertyName, {headerValue: Value, enableResize: boolean, toValue: ToValueFunction}>}
       */
      ...
      _getDataDirectionCells : function() {
        ...
        var totalRowCountType = typeof this._param.totalRowCount;
        if (totalRowCountType === 'number') {
          mainRows = this._param.totalRowCount;
        } else if (totalRowCountType === 'function') {
          mainRows = this._param.totalRowCount(this.getSearcher());
        } else {
          if (typeof this._param.rowCount === 'number') {
            // 全体行数が指定されていなくても、
            // rowCountが数値で固定指定されていれば、
            // Searcher経由でデータの個数をもらって行数を計算できる。
            // これにより、rowCountを数値で指定した場合は、
            // totalRowCountは不要。
            mainRows = this.getSearcher().getCount() * this._param.rowCount;
          } else {
            // totalRowCountもrowCount(数値指定)もない場合はデータの行数とみなす(1:1マッピング)
            mainRows = this.getSearcher().getCount();
          }
        }

        return mainRows + depth;
      },
       ...
       * @private
       */
      _dispatchChangePropertySizeEvent : function() {
        this.dispatchEvent({
          type : 'changePropertySize'
        });
      },
      ...
      /**
       * TODO このグリッドでは必ず縦方向が行、横方向が列だが、このメソッド名は このルールに従っていないのでPropertyNumなどと直すのがよい
       * 
       * @private
       */
      _getSubRowNumOfRecord : function(row, column, depth, dataIndex) {
        // rowNumは0-origin
        var rowNum = this._isVertical() ? row : column;
        ...
        var subRowNum = 0;
        if (typeof this._param.rowCount === 'number') {
          var rowCount = this._param.rowCount;
          subRowNum = rowNum % rowCount;
        } else if (typeof this._param.rowCount === 'function') {
          var rc = this._param.rowCount(dataIndex, this._searcher);
          // firstRowNumも0-origin
          var firstRowNum = rc.firstRowNum;
          subRowNum = rowNum - firstRowNum;
        }
        return subRowNum;
      },
       ...
       * @private
       */
      _makeDataCell : function(properties, row, column, depth, index, mergeTable) {
        var pIndex = this._isVertical() ? column : row;
        var property = properties[pIndex];
        var propertyBase = properties[pIndex];

        var subRowNum = this._getSubRowNumOfRecord(row, column, depth, index);

        if (subRowNum !== 0) {
          // サブ行用のプロパティ名に直す
          property = propertyBase + '/' + String(subRowNum);
        }

        var result = null;

        var isPhantom = mergeTable[subRowNum][propertyBase] != null;

        var isBlank = typeof this._propertyDefinitionSet[property] === 'undefined';
        ...
        result = {
          ...
          baseCellInfo : mergeTable[subRowNum][propertyBase],
          isHeaderRow : !this._isVertical() && (row < this.getHeaderRows()),
          isHeaderColumn : this._isVertical() && (column < this.getHeaderColumns()),
          isLoaded : false
        };

        if (this._propertyDefinitionSet[property]) {
          var merge = this._propertyDefinitionSet[property].merge;
          if (typeof merge === 'function') {
            var cellSpan = merge(index, this._searcher);
            if (cellSpan.rowSpan != null) {
              result.rowSpan = cellSpan.rowSpan;
            }
            if (cellSpan.colSpan != null) {
              result.colSpan = cellSpan.colSpan;
            }
          } else if (merge) {
            // 固定値がオブジェクトで渡されている場合
            if (merge.rowSpan != null) {
              result.rowSpan = merge.rowSpan;
            }
            if (merge.colSpan != null) {
              result.colSpan = merge.colSpan;
            }
          }
        }
      ...
      _applyToValue : function(cell) {
        var propDef = this._propertyDefinitionSet[cell.propertyName];
        if (!propDef) {
          return;
        }
        var toValue = propDef.toValue;
        cell.originalValue = cell.originalData == null ? null : toValue(cell.originalData, cell);
      ...
      /**
       * インデックスのオフセットを計算します。ここでいうオフセットとは、 セルが結合されている場合に、可視範囲の先頭の行番号と結合の境界の行番号の差のことです。
       * 
       * @param startRow
       * @param startColumn
       * @param depth
       * @returns {___anonymous359085_359134}
       */
      _getOffsetIndex : function(startRow, startColumn, depth) {
        var rowCountType = typeof this._param.rowCount;

        var offsetTop = 0;
        var offsetLeft = 0;

        var row = startRow - depth;

        if (rowCountType === 'number') {
          var rowCount = this._param.rowCount;
          offsetTop = row % rowCount;
        } else if (rowCountType === 'function' && this._searcher.isReady()) {
          var dataIndex = this._param.rowToDataIndex(startRow - depth, this._searcher);
          var rowInfo = this._param.rowCount(dataIndex, this._searcher);
          offsetTop = row - rowInfo.firstRowNum;
        }
      ...
      /**
       * あるデータ行に対応するセル範囲について、セルの結合状態を計算します。 結果は2次元配列で表現され、各要素は、結合されていない、または結合の親セルならtrue、 結合されて隠れているセル（ファントムセル）の場合はfalseです。
       * 
       * @private
       * @param properties リーフプロパティの、現在の
       */
      _createCellMergeTable : function(dataIndex, properties) {
        var that = this;
        ...
        var rowCountType = typeof this._param.rowCount;
        if (rowCountType === 'number') {
          rowsPerData = this._param.rowCount;
        } else if (rowCountType === 'function') {
          var rowInfo = this._param.rowCount(dataIndex, this._searcher);
          rowsPerData = rowInfo.rowCount;
        }

        // 列: 行の2次元配列 [ 行0[列Prop0, 列Prop1, ...], 行1[], ...] xx
        // 行: 列 -> [ 行0{列Prop0: true, 列Prop1: false, ...} , 行1[], ...] xx
        // 中身は結合されているかどうかのBoolean
        var mergeTable = [];

        for (var subRowIdx = 0; subRowIdx < rowsPerData; subRowIdx++) {
          mergeTable[subRowIdx] = {};

          for (var pIdx = 0, pLen = properties.length; pIdx < pLen; pIdx++) {
            var propertyBase = properties[pIdx];

            mergeTable[subRowIdx][propertyBase] = null;
          }
        }
        ...
        for (var subRowIdx = 0; subRowIdx < rowsPerData; subRowIdx++) {
          for (var pIdx = 0, pLen = properties.length; pIdx < pLen; pIdx++) {
            ...
            if (mergeTable[subRowIdx][propertyBase] != null) {
              // 左上から順に走査していく中で、
              // あるセルのrowSpan,colSpanに1より大きい値が設定されていたとしても
              // そのセルがすでにファントムと分かっていたら無視する
              continue;
            }

            var propKey = subRowIdx === 0 ? propertyBase : propertyBase + '/' + String(subRowIdx);

            var cellSpan = getCellSpan(dataIndex, propKey);

            for (var rowSpan = 0; rowSpan < cellSpan.rowSpan; rowSpan++) {
              for (var colSpan = 0; colSpan < cellSpan.colSpan; colSpan++) {
                if (rowSpan === 0 && colSpan === 0) {
                  continue;
                }

                var mergeTargetProp = properties[pIdx + colSpan];
                var baseCellInfo = {
                  subRowNum : subRowIdx,
                ...
                mergeTable[subRowIdx + rowSpan][mergeTargetProp] = baseCellInfo;
              }
            }
          }
        }
        ...
        function getCellSpan(dataIndex, property) {
          if (!that._propertyDefinitionSet[property]) {
            return {
              rowSpan : 1,
              colSpan : 1
            };
          ...
          var merge = that._propertyDefinitionSet[property].merge;
          var rowSpan = 1;
          var colSpan = 1;
          if (typeof merge === 'function') {
            var cs = merge(dataIndex, that._searcher);
            if (cs.rowSpan != null) {
              rowSpan = cs.rowSpan;
            }
            if (cs.colSpan != null) {
      ...
      /**
       * セルが結合されている場合に、後ろ方向に表示されていない（残りの）行数を返します。
       * 
       * @private
       * @param lastDataRowNum データ部分の行番号（先頭のデータ行が0、ヘッダ部分を含まない）
       * @param depth
       * @returns {Number}
       */
      _getRowRemainder : function(lastDataRowNum, depth) {
        if (lastDataRowNum === 0) {
          return 0;
        }
        ...
        if (lastDataRowNum === this._getDataDirectionCells() - depth) {
          // 最終行の場合は余りは必ず0
          rowRemainder = 0;
        } else if (typeof this._param.rowCount === 'number') {
          var rowCount = this._param.rowCount;
          rowRemainder = rowCount - lastDataRowNum % rowCount - 1;
        } else if (typeof this._param.rowCount === 'function') {
          var dsIdx = this._param.rowToDataIndex(lastDataRowNum, this._searcher);
          var rc = this._param.rowCount(dsIdx, this._searcher);
          // firstRowNumは0-origin、データ行の先頭が0
          rowRemainder = rc.rowCount - (lastDataRowNum - rc.firstRowNum);
        }

        return rowRemainder;
      },

      _getHeaderFirstRootColumnIndex : function(properties, columnIndex) {
        var isInHeader = true;
        var headerStructure;
        if (columnIndex < this._getHeaderCells()) {
          headerStructure = this._headerProperties.getHeaderStructure();
        } else {
          isInHeader = false;
          headerStructure = this._mainProperties.getHeaderStructure();
        }

        var firstColumnName = properties[columnIndex];

        for (var stIdx = 0, stLen = headerStructure.length; stIdx < stLen; stIdx++) {
          var propDepthArray = headerStructure[stIdx];
          for (var stDepth = 0, stDepthLen = propDepthArray.length; stDepth < stDepthLen; stDepth++) {
            var propInfo = propDepthArray[stDepth];
            if (propInfo.isBaseCell && propInfo.property === firstColumnName) {

              for (var rootIdx = stIdx; rootIdx >= 0; rootIdx--) {
                var rootPropInfo = headerStructure[rootIdx][0];
                if (rootPropInfo.isBaseCell) {
                  // ヘッダの場合はそのままだが、メイン部分のヘッダの場合は、ヘッダ分のセル数を足す必要がある
                  var ret = isInHeader ? rootIdx : rootIdx + this._getHeaderCells();
                  return ret;
                }
              }
        ...
        // ここには到達しないはず
        throw new Error('getHeaderFirstRootColumnIndex: root not found error');
      },
      ...
      _mapRange : function(param) {
        ...
        var dataDirectionSize = this._param.dataDirectionSize.size;
        ...
        if (!this._isPropertyHeaderCell(range.rowIndex, range.columnIndex) && range.rowLength > 0) {
          offsetIndex = this._getOffsetIndex(range.rowIndex, 1, depth);
          offset = {
            top : -dataDirectionSize * offsetIndex.top,
            left : 0
          };

          var lastDataRowNum = range.rowIndex + range.rowLength - depth;
          rowRemainder = this._getRowRemainder(lastDataRowNum, depth);
        } else {
          // ヘッダセルの場合、見えているリーフセルを含むルートセルの先頭から右端（可視範囲最後）まで描画する
          var firstRootColumnIndex = this._getHeaderFirstRootColumnIndex(properties, range.columnIndex);

          var orgColIndex = range.columnIndex;
          // コメントアウトしてある方だと、常に先頭列から描画する
          // offsetIndex.left = range.columnIndex - (range.columnIndex < this._getHeaderCells() ? 0 : this._getHeaderCells());
          offsetIndex.left = range.columnIndex - firstRootColumnIndex;
          range.columnLength += offsetIndex.left;
          // range.columnIndex = range.columnIndex < this._getHeaderCells() ? 0 : this._getHeaderCells();
          range.columnIndex = firstRootColumnIndex;

          var headerColumnWidthSet = this.getColumnWidthSet();
          var colWidthIdx = 0;

          for (var i = range.columnIndex, len = orgColIndex; i < len; i++) {
            colWidthIdx += headerColumnWidthSet.toSizeFromIndex(i);
          }
        ...
        var mergeTable;
        var lastDataSourceIndex = -1; // データ行は必ず0以上

        var cells = [];
        for (i = 0, rLen = range.rowLength + offsetIndex.top + rowRemainder; i < rLen; i++) {
          var row = i + range.rowIndex - offsetIndex.top;

          var dataSourceIndex = row - depth;

          // マージテーブルの作成を必要最小限に抑えてパフォーマンスを向上させるため、
          // 行方向にデータが伸びていくことを前提に、行ループの中、かつ
          // データ行が変わったタイミングでのみマージテーブルを再計算する。
          // 列方向にデータが伸びていく場合、これだと正しく動作しないので注意
          if (dataSourceIndex >= 0) {
            /* rowCount追加への対応 */
            var rowCountType = typeof this._param.rowCount;
            if (rowCountType === 'number') {
              var rowCount = this._param.rowCount;
              dataSourceIndex = Math.floor((row - depth) / rowCount);
            } else if (rowCountType === 'function') {
              dataSourceIndex = this._param.rowToDataIndex(row - depth, this._searcher);
            }

            if (dataSourceIndex !== lastDataSourceIndex) {
              // 1回前のループと同じデータ行について処理する場合は、mergeTableは同じになるので再計算しない
              lastDataSourceIndex = dataSourceIndex;
              mergeTable = this._createCellMergeTable(dataSourceIndex, properties);
            }
          }
          ...
          for (j = 0, cLen = range.columnLength; j < cLen; j++) {
            ...
            if (isPropertyHeader) {
              ...
              cell = this._makeDataCell(properties, row, column, depth, dataSourceIndex, mergeTable);

              var data = dataArray[dataSourceIndex - startIndex];
              if (data.isLoaded) {
        ...
        var rowHeightSet = this._isVertical() ? this._dataDirectionSizeSet : this._propertyDirectionSizeSet;
        var columnWidthSet = this._isVertical() ? this._propertyDirectionSizeSet : this._dataDirectionSizeSet;
        ...
        var gridRange = new GridRange({
          ...
          top : offset.top, // ピクセル指定でのオフセット
          left : offset.left,
          offsetIndex : offsetIndex, // インデックス指定でのオフセット
          cells : cells,
  ...
  function dispatchChangeGridEvent(changes) {
    ...
    this._scrollRowHeightSet.setHeaderCells(this.getHeaderRows());
    this._scrollColumnWidthSet.setHeaderCells(this.getHeaderColumns());
    this.dispatchEvent({
      type : 'changeGrid',
      changes : changes
    });
  ...
  var logicEventListeners = {
    ...
    _dataMapper : {
      ...
      changePropertySize : changeGridHandler,
      changeDataCount : changeGridHandler
    },
  ...
  var gridLogicDefinition = {
    ...
    init : function(dataSource, param) {
      ...
      var cellSelector = new CellSelector(this.own(this.getGridCell));
      ...
      if (param.searcher.type === 'all') {
        ...
        searcher = new datagrid.data._privateClass.LazyFetchSearcher(dataSource, searcherParam);
      }

      // ページング機能付きDataSearcherでwrapする
      if (typeof param.searcher.paging !== 'undefined' && param.searcher.paging.enable) {
        searcher = new datagrid.data._privateClass.PagingDataSearcher(searcher, param.searcher.paging.pageSize);
      }

      var mapperParam = param.mapper.param;
      var mapper = new PropertyGridDataMapper(searcher, dataFocus, dataSelector, cellSelector, mapperParam);
    ...
    calcVerticalDisplayRange : function(displayAreaHeight, verticalPos) {
      var range = this._verticalScrollCalculator.calcDisplayRange(displayAreaHeight, verticalPos);
    ...
    calcHorizontalDisplayRange : function(displayAreaWidth, horizontalPos) {
      var range = this._horizontalScrollCalculator.calcDisplayRange(displayAreaWidth, horizontalPos);
    ...
    calcPrevHorizontalPosition : function(displayAreaWidth, horizontalPos) {
      return this._horizontalScrollCalculator.calcPrevPosition(displayAreaWidth, horizontalPos);
    },
    ...
    calcNextHorizontalPosition : function(displayAreaWidth, horizontalPos) {
      return this._horizontalScrollCalculator.calcNextPosition(displayAreaWidth, horizontalPos);
    },
    ...
    getGridCell : function(row, column) {
      ...
      var offsetIndex = range.getOffsetIndex();
      return range.getCell(offsetIndex.top, offsetIndex.left);
    },
...
(function() {
  ...
  function getCellHeight(gridRange, startRowIdx, rowSpan) {
    var cellHeight = 0;
    for (var i = startRowIdx, len = startRowIdx + rowSpan; i < len; i++) {
      cellHeight += gridRange.getRowHeight(i);
    }
    return cellHeight;
  }

  function getCellWidth(gridRange, startColIdx, colSpan) {
    var cellWidth = 0;
    for (var i = startColIdx, len = startColIdx + colSpan; i < len; i++) {
      cellWidth += gridRange.getColumnWidth(i);
    }
    return cellWidth;
  }
    ...
    return function($target, gridRange) {
      ...
      for (i = 0; i < rowLength; i++) {
        ...
        for (j = 0; j < columnLength; j++) {
          ...
          if (cell.isPhantom) {
            // このセルは結合により消え去っている(隠れている)セル
            // ⇒これに対応する<td>は出力しない(rowspan, colspan等で最終的につぶれる)
            continue;
          }

          var rowSpan = cell.rowSpan;
          var colSpan = cell.colSpan;
          var cellHeight = getCellHeight(gridRange, i, cell.rowSpan);
          // TODO cellHeightと名前を合わせてcellWidthにした方がよい
          var width = getCellWidth(gridRange, j, cell.colSpan);

          // TODO: L字のセル結合を考慮する

          var isFirstRow = cell.row === 0;
          var isFirstColumn = cell.column === 0;
          var isLastRow = cell.row === gridRange.getAllRows() - 1 || cell.row + rowSpan === gridRange.getAllRows();
          var isLastColumn = cell.column === gridRange.getAllColumns() - 1 || cell.column + colSpan === gridRange.getAllColumns();

          // MEMO: 結合の親セルの場合は隠しているセルの border を考慮する
          var topBorders = [];
          for (var k = j; k < j + colSpan; k++) {
            topBorders.push(gridRange.getTopBorder(i, k));
          }
          var leftBorders = [];
          for (k = i; k < i + rowSpan; k++) {
            leftBorders.push(gridRange.getLeftBorder(k, j));
          }
          var bottomBorders = [];
          for (k = j; k < j + colSpan; k++) {
            var bottomBorder = gridRange.getBottomBorder(i + (rowSpan - 1), k);
            ...
            bottomBorders.push(bottomBorder);
          }
          var rightBorders = [];
          for (k = i; k < i + rowSpan; k++) {
            var rightBorder = gridRange.getRightBorder(k, j + (colSpan - 1));
            if (rightBorder == null) {
              rightBorder = {
                classes : [],
                width : 0
          ...
          var topBorderWidth = Math.floor(topBorders[0].width / 2);
          var leftBorderWidth = Math.floor(leftBorders[0].width / 2);
          var bottomBorderWidth = Math.ceil(bottomBorders[0].width / 2);
          var rightBorderWidth = Math.ceil(rightBorders[0].width / 2);

          if (isFirstRow) {
            topBorderWidth = topBorders[0].width;
          }
          if (isFirstColumn) {
            leftBorderWidth = leftBorders[0].width;
          }
          if (isLastRow) {
            bottomBorderWidth = bottomBorders[bottomBorders.length - 1].width;
          }
          if (isLastColumn) {
            rightBorderWidth = rightBorders[rightBorders.length - 1].width;
          }

          // FIXME
          var divTop = 0;
          var divLeft = 0;
          var divHeight = cellHeight;
          var divWidth = width;
          ...
          var cssClass = [ "gridCell" ]
          if (cell.propertyName) {
            cssClass.push(cell.propertyName)
          }
          html += `<div class="${cssClass.join(' ')}" style="overflow: hidden; position: absolute;`;
          html += ' top:' + divTop + 'px;';
          ...
          if (0 < topBorderWidth) {
            // 結合しているセルの数(colSpan)の数だけ topBorder を追加する
            borderLeft = 0;
            for (cnt = 0; cnt < topBorders.length; cnt++) {
              if (topBorders[cnt] == null) {
                break;
              }
              html += '<div class="gridBorder gridHorizontalBorder gridTopBorder';
              util.forEach(topBorders[cnt].classes, function(cls) {
                html += ' ';
                html += cls;
              });

              borderHeight = isFirstRow ? topBorderWidth : Math.floor(topBorders[cnt].width / 2);
              borderWidth = gridRange.getColumnWidth(j + cnt);

              html += '" style="';
              html += ' position: absolute;';
              html += ' top: 0;';
              html += ' left: ';
              html += borderLeft === 0 ? '0;' : borderLeft + 'px;';
              html += ' height: ' + borderHeight + 'px;';
              html += ' width: ' + borderWidth + 'px;';
              html += '"></div>';
          ...
          if (0 < leftBorderWidth) {
            // 結合しているセルの数(rowSpan)の数だけ leftBorder を追加する
            borderTop = 0;
            for (cnt = 0; cnt < leftBorders.length; cnt++) {
              if (leftBorders[cnt] == null) {
                break;
              }
              html += '<div class="gridBorder gridVerticalBorder gridLeftBorder';
              util.forEach(leftBorders[cnt].classes, function(cls) {
                html += ' ';
                html += cls;
              });

              borderHeight = gridRange.getRowHeight(i + cnt);
              borderWidth = isFirstColumn ? leftBorderWidth : Math.floor(leftBorders[cnt].width / 2);

              html += '" style="';
              html += ' position: absolute;';
              html += ' top: ';
              html += borderTop === 0 ? '0;' : borderTop + 'px;';
              html += ' left: 0;';
              html += ' height: ' + borderHeight + 'px;';
              html += ' width: ' + borderWidth + 'px;';
              html += '"></div>';
          ...
          if (0 < bottomBorderWidth) {
            // 結合しているセルの数(colSpan)の数だけ bottomBorder を追加する
            borderLeft = 0;
            for (cnt = 0; cnt < bottomBorders.length; cnt++) {
              if (bottomBorders[cnt] == null) {
                break;
              }
              html += '<div class="gridBorder gridHorizontalBorder gridBottomBorder';
              util.forEach(bottomBorders[cnt].classes, function(cls) {
                html += ' ';
                html += cls;
              });

              borderHeight = isLastRow ? bottomBorderWidth : Math.ceil(bottomBorders[cnt].width / 2);
              borderWidth = gridRange.getColumnWidth(j + cnt);
          ...
          if (0 < rightBorderWidth) {
            // 結合しているセルの数(rowSpan)の数だけ rightBorder を追加する
            borderTop = 0;
            for (cnt = 0; cnt < rightBorders.length; cnt++) {
              if (rightBorders[cnt] == null) {
                break;
              }
              html += '<div class="gridBorder gridVerticalBorder gridRightBorder';
              util.forEach(rightBorders[cnt].classes, function(cls) {
                html += ' ';
                html += cls;
              });

              borderHeight = gridRange.getRowHeight(i + cnt);
              borderWidth = isLastColumn ? rightBorderWidth : Math.ceil(rightBorders[cnt].width / 2);
          ...
          if (cell.isHeaderRow && cell.enableResize) {
            html += resizeColumnUI.makeResizeMarkerHtml(cellHeight);
          }
  ...
  var horizontalScrollBarController = {
    ...
    __construct : function() {
      ...
      this._pressScrollAreaRightRepeat = util.createRepeat(firstWait, interval, pressAreaRight);
  ...
  var columnSortAndFilterUIController = {
    ...
    '{.gridFilterButton} click' : function() {
      ...
      if ($menu.find('.gridFilterOptionsItem').is(':visible')) {
        ...
        // 全選択されている場合はクリアと同様の処理とする。
        if ($checkboxes.length - 1 == selected.length) {
  ...
  var tableGridViewController = {
    ...
    __construct : function() {
      ...
      var keydownTab = this.own(this._triggerKeydownTab);
      this._keydownTabThrottle = util.createThrottle(KEYDOWN_WAIT_TIME, keydownTab, {
        deferFirst : true,
        deferInWaiting : false
      });
    ...
    '{rootElement} keydown' : function(context) {
      ...
      if (keycode === 9) {
        ...
        if (event.shiftKey) {
          // SHIFT+TAB: 一番左上の場合は通常の処理とする
          if (column <= 0 && row <= 0) {
            // 次にフォーカスがうつったときに右下に移動する
            this._nextFocusCell = {
              row : totalRows - 1,
          ...
          // TAB: 一番右下の場合は通常の処理とする
          if (totalColumns - 1 <= column && totalRows - 1 <= row) {
            // 次にフォーカスがうつったときに左上に移動する
            this._nextFocusCell = {
              row : 0,
              column : 0
            };
        ...
        event.preventDefault();
        this._keydownTabThrottle.call(event);
        return;
      }
    ...
    '.gridCellFrame mousedown' : function(context, $el) {
      ...
      if (!$target.is($el) && $target.parent().is(':input')) {
        return;
      }
      if ($target.is('a')) {
        return;
    ...
    _triggerKeydownTab : function(event) {
      var focusedCell = this._gridLogic.getFocusedCell();
      ...
      // TAB: 一番右下の場合は何もしない（本来はここには入ってこないはず）
      if (!event.shiftKey && totalColumns - 1 <= column && totalRows - 1 <= row) {
        return;
      }

      // SHIFT+TAB: 一番左上の場合は何もしない（本来はここには入ってこないはず）
      if (event.shiftKey && column <= 0 && row <= 0) {
        return;
      }

      // それ以外ではフォーカスをひとつ横にずらす
      column += event.shiftKey ? -1 : 1;

      // 右端を越えた場合はひとつ下の行の左端に移動する
      if (totalColumns <= column) {
        row += 1;
        column = 0;
      }
      // 左端を越えた場合はひとつ上の行の右端に移動する
      if (column < 0) {
        row -= 1;
        column = totalColumns - 1;
      }
    ...
    _selectRangeScroll : function(x, y) {
      ...
      // 範囲選択した領域が列固定した領域に収まる場合はx方向への移動量は0とする
      var range = this._gridLogic.getSelectedRange();
      var columnIndex = range.columnIndex;
      var columnLength = range.columnLength;
      var visibleProperties = this._gridLogic.getVisibleProperties();
      var header = visibleProperties.header;
      var isTypeOfScrollDiff = !!evArg.horizontal && evArg.horizontal.type === 'scrollDiff';
      if (isTypeOfScrollDiff && columnIndex + columnLength <= header.length) {
        // 範囲選択の終点が列固定領域に収まっていれば
        // x方向のスクロールはしない
        evArg.horizontal.diff = 0;
      }
...
(function() {
  ...
  var gridController = {
    ...
    getPropertySize : function(propertyName) {
      var size = this._gridLogic._dataMapper.getPropertySize(propertyName);
      return size;
    },

    setPropertySize : function(propertyName, size) {
      this._gridLogic._dataMapper.setPropertySize(propertyName, size);
    },
     ...
     * @private
     */
    _createPropertyHierarchy : function(properties, hierarchy) {
      var hr = $.extend(true, {}, hierarchy);

      var childKeys = getKeyArrayDeeply(hr);

      for ( var prop in properties) {
        if ($.inArray(prop, childKeys) === -1) {
          // childKeysの中にpropが存在しないので
          // ヒエラルキーのルートに存在させる
          hr[prop] = null;
        }
      }
      ...
      function getKeyArrayDeeply(obj) {
        var keys = [];
        for ( var k in obj) {
          keys.push(k);

          if ($.isPlainObject(obj[k])) {
            var childKeys = getKeyArrayDeeply(obj[k]);
    ...
    _parseParam : function(param) {
      ...
      if (!!param.propertyHierarchy) {
        propertyHierarchy = this._createPropertyHierarchy(properties, param.propertyHierarchy);
      } else {
