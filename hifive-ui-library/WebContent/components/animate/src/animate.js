function animate(element, params, option){
	...
	if(!isTransition){
		...
		complete = function(){
			...
			if(_option.complete) _option.complete();
		};
...
function convertToTranslate(element, params, option){
	...
	if(transform){
		...
		if(match){
			...
			match = transform.match(/(matrix3d\((?:[^,]+, *){12})([^,]+), *([^,]+), *((?:[^,]+, *){2}\))/i);
			if(match){
				left = _getLT(_params.left, match[2]);
				top = _getLT(_params.top, match[3]);
	...
	function _getLT(value, current){
		if(value != null){
			if(typeof value == 'string'){
				...
				if(match){
					return parseFloat(current) + (parseFloat(match[2]) * (match[1] == '+' ? 1 : -1));
				}else{
					return value;
				}
			}else{
...
function addPosition(element, params){
	...
	$.each(['left', 'top'], function(){
		if(!(this in _params)) return;
		var paramsLT = _params[this];
		if(typeof paramsLT != 'string') return;
